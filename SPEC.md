...
## 9.3 現在の移行完了状態
- 関東、信越、東北の路線実体定義は、現時点で `window.railwayLineFileChunks` 側へ移行済み。
- 実際の路線定義の正本は、`station_metadata/lines/*.js` および `station_metadata/line_*.js`、`station_metadata/group_*.js` にある。
- `data_kanto.js`、`data_tohoku.js`、`data_shinetsu.js` は、互換性維持のために残している空ファイルとして扱う。
- 現運用では、これら `data_*.js` に新規の路線定義・駅定義を追加しない。
- 今後の路線修正は、原則として該当する `station_metadata` 配下のファイルのみを編集対象とする。
- 例外的に `data_*.js` を触るのは、互換レイヤの維持や読込順調整が必要な場合に限定する。
